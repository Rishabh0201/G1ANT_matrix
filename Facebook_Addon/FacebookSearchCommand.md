...
```G1ANT
facebook.search type ⟦text⟧
```

## Description
This command search any product on facebook.
...
> **Note:** the `facebook.` commands require opening a browser with the `facebook.open` command first.
## Example

In the following example the browser waits until facebook webpage is loaded, After 5 second starts login in Flipkart acount using given credential. after 5 second start searching given text in the search box:

```G1ANT
facebook.open 
delay 5
facebook.login email xxxxxx Pass xxxx search
delay 5
facebook.search "headphone" search
```
