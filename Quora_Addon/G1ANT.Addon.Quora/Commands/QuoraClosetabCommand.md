...
This command is used to closes the current tab in the current browser in which qoura is opened.
...
> **Note:** the `qoura.` commands require opening a browser with the `qoura.open` command first.

## Example

In the following script the robot opens Qoura tabs in Chrome, and after 5 seconds the current tab is closed.
