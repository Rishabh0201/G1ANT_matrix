...
This command is used to closes the current tab in the current browser in which olacab is opened.
...
> **Note:** the `olacab.` commands require opening a browser with the `olacab.open` command first.

## Example

In the following script the robot opens OlaCab tabs in Chrome, and after 5 seconds the current tab is closed.
