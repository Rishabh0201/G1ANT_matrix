...
## Description
This command logout from naukri web.
...
> **Note:** the `naukri.` commands require opening a browser with the `naukri.open` command first then login in your account.
## Example
In the following example the browser waits until naukri webpage is loaded, but then the robot opens naukri and After 5 second starts login in line acount using given credential. After 5 secon it logout from your Account:
```G1ANT
naukri.open 
delay 5
naukri.login email xxxxxx Pass xxxx search
delay 5
naukri.logout 
```
